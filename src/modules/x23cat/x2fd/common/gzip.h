...
class gzwriter
{
  ...
  private:
    static const size_t growby=1024;    // bytes to grow output stream
    ...
    value_type * ptr() const { return m_ptr; }
    size_t allocated() const { return m_allocated; }
    size_t avail() const { return allocated() - (ptr() - buffer()); }
    bool allocate(size_t newsize);
    void error(int err) { m_nError=err; }

  public:
    gzwriter() { memset(this, 0, sizeof(*this)); m_complevel=
    Z_DEFAULT_COMPRESSION; }
    ~gzwriter() { delete[] m_buffer; delete[] m_pszComment, delete[]
    m_pszFileName; }
    const char * fileName() const { return m_pszFileName; }
    void fileName(char *pszName) { strcreate(m_pszFileName, pszName); }

    int error() const { return m_nError; }

    const char * comment() const { return m_pszComment; }
    void comment(char *pszText) { strcreate(m_pszComment, pszText); }

    int compression() const { return m_complevel; }
    void compression(int val) { m_complevel=val; }
    ...
    bool pack(value_type *buffer, size_t size);
};
